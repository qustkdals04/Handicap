            ...
            <c:forEach var="commentList" items="${list}">
               <tr>
                  ...
                  <c:if test="${bbsContent.nickname == membernick }">
                     <td align="center"><img src="/Handicap/img/temp.jpg" onclick="location.href='/Handicap/commentDelete?no=${bbsContent.no}&boardno=${bbsContent.boardno}&commentno=${commentList.commentno }'"></td>
                  </c:if>
               </tr>
            </c:forEach>
            <tr>
            ...
            <table>
               ...
               <td style="font-size: large;">
                  <c:if test="${startPage>1 }">
                     <span>
                        <a href="/Handicap/bbsContent?pageNumber=${startPage-1 }&boardno=${bbsContent.boardno}&no=${bbsContent.no}">이전</a>
                     </span>
                  </c:if>
                  <c:forEach var="i" begin="${startPage }" end="${endPage }">
                     <c:choose>
                        <c:when test="${pageNumber==i }">
                           <span>
                              <a href="/Handicap/bbsContent?pageNumber=${i }&boardno=${bbsContent.boardno}&no=${bbsContent.no}" style="text-decoration: none; color: blue; font-weight: bold;">${i}</a>&nbsp;
                           </span>
                        </c:when>
                        <c:otherwise>
                           <span>
                              <a href="/Handicap/bbsContent?pageNumber=${i}&boardno=${bbsContent.boardno}&no=${bbsContent.no}"   style="text-decoration: none; color: gray;">${i}</a>&nbsp;
                           </span>
                        </c:otherwise>
                     </c:choose>
                  </c:forEach>
                  <c:if test="${endPage<totalPageCount}">
                     <span>
                        <a href="/Handicap/bbsContent?pageNumber=${endPage+1 }&boardno=${bbsContent.boardno}&no=${bbsContent.no}">다음</a>
                     </span>
                  </c:if>
               </td>
            </tr>
            ...
            <table>
									...
									<tr>
										<td width="700px" colspan="2"><input type="text"
											style="width: 500px; height: 50"><input type="button"
											value="등록"
